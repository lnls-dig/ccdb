 ...
 */
class InstallationSlotNameClient implements InstallationSlotNameResource {

    private static final Logger LOG = Logger.getLogger(InstallationSlotNameClient.class.getName());

    private static final String PATH_SLOT_NAMES = "names";
    ...
    public InstallationSlotNameList getAllInstallationSlotNames(String deviceTypeName) {
        ...
        try (final ClosableResponse response = client.getResponse(url)) {
            return response.readEntity(InstallationSlotNameList.class);
        } catch (Exception e) {
